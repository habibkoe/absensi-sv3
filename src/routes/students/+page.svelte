  ...
  async function handleFileUpload(event: Event) {
    ...
    try {
      ...
      const processedData = jsonData.map((row: any, index: number) => {
        ...
        const registration_number = normalizedRow.registration_number || normalizedRow.nis || normalizedRow.nomor_induk || '';
        let gender = normalizedRow.gender || normalizedRow.jenis_kelamin || normalizedRow['jenis kelamin'] || '';
        ...
        if (gender) {
          ...
          if (gender === 'L' || gender === 'LAKI-LAKI' || gender === 'LAKI' || gender === 'M' || gender === 'MALE') {
            gender = 'L';
          } else if (gender === 'P' || gender === 'PEREMPUAN' || gender === 'F' || gender === 'FEMALE') {
            gender = 'P';
          } else {
            gender = '';
          }
      ...
      importError = 'Failed to parse Excel file. Please ensure it is a valid Excel file (.xlsx or .xls)';
    }
  }
    ...
    <div class="flex items-center justify-between mb-8">
      ...
      <div class="flex gap-3">
        <button
          ...
          class="px-4 py-2 font-semibold text-white transition-colors bg-green-600 rounded-lg hover:bg-green-700"
        >
          📥 Import Data Siswa
        </button>
        <button
...
{#if showImportModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto bg-black bg-opacity-50">
    <div class="w-full max-w-4xl p-6 my-8 bg-white rounded-lg shadow-xl">
      <h2 class="mb-4 text-2xl font-bold text-gray-800">Import Siswa dari Excel</h2>
      ...
      <div class="space-y-4">
        ...
        <div class="p-4 border-l-4 border-blue-500 bg-blue-50">
          ...
          <ul class="text-sm text-blue-800 list-disc list-inside">
            <li>Column headers: <strong>name</strong>, <strong>gender</strong>, <strong>registration_number</strong></li>
        ...
        <!-- Classroom Selection -->
        <div>
          <label for="importClassroom" class="block mb-2 text-sm font-medium text-gray-700">
            Pilih ruang kelas * (Data siswa yang diimpor akan dimasukkan ke dalam ruang kelas ini)
          </label>
          <select
            id="importClassroom"
        ...
        <!-- File Upload -->
        <div>
          <label for="importFile" class="block mb-2 text-sm font-medium text-gray-700">
          ...
          />
          {#if !importClassroomId}
            <p class="mt-1 text-xs text-gray-500">Mohon pilih ruang kelas terlebih dahulu.</p>
          {/if}
        </div>
        ...
        {#if showPreview && importPreview.length > 0}
          <div class="mt-4">
              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    ...
                    <th class="px-4 py-2 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Registration Number</th>
                    <th class="px-4 py-2 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Gender</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  {#each importPreview as row}
                    <tr class={row.valid ? '' : 'bg-red-50'}>
                      <td class="px-4 py-2 text-sm text-gray-500 whitespace-nowrap">{row.rowNumber}</td>
                      <td class="px-4 py-2 text-sm text-gray-900 whitespace-nowrap">{row.name}</td>
                      <td class="px-4 py-2 text-sm text-gray-900 whitespace-nowrap">{row.registration_number}</td>
                      ...
                      <td class="px-4 py-2 text-sm whitespace-nowrap">
                        {#if row.valid}
                          <span class="px-2 py-1 text-xs font-semibold text-green-800 bg-green-100 rounded-full">✓ Valid</span>
                        {:else}
                          <span class="px-2 py-1 text-xs font-semibold text-red-800 bg-red-100 rounded-full">✗ Invalid</span>
            ...
            <p class="mt-2 text-sm text-gray-600">
              Total: {importData.length} rows | 
              Valid: {importData.filter(d => d.valid).length} | 
              Invalid: {importData.filter(d => !d.valid).length}
            </p>
          </div>
        {/if}
        ...
        <div class="flex justify-end space-x-3">
          ...
          {#if showPreview && importData.filter(d => d.valid).length > 0}
            <button
              ...
              disabled={importing}
            >
              {importing ? 'Importing...' : `Import ${importData.filter(d => d.valid).length} Student(s)`}
            </button>
          {/if}
        </div>
      </div>
